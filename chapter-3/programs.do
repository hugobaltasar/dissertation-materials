...
////////////////////////////////////////////////////////////////////////////////
////////////////////InAgg with EU-SILC - Auxiliary programs/////////////////////
////////////////////////////////////////////////////////////////////////////////
...
capture program drop iopgini
*! This program is based on version 2.6 (18th dec 2015) of iop by F. Chavez Juarez & Isidro Soloaga
program define iopgini, rclass
...
if("`type'"==""){ // Find the type of the varible
	tempvar check
	gen `check'=`depvar'
	qui: compress `check'
...
if(`case'==1){ // DICHOTOMOUS
	quietly{
    	...
    	if(`bootstrap' & `bootstrap'>0){ // Run the bootstrap if needed
    		noisily di as text "Bootstrapping..." _continue
			forvalues i=1/`bootstrap'{
    			preserve
    			qui: keep if `touse'
    			bsample, `bootopt'
    			iopgini `depvar' `indepvars' if `touse' [`weight'`exp'], `logit'
    			if(`i'==1){
    ...
    quietly{
		...
		fastgini `yhat' if `touse' [`weight'`exp']
		local res_FGa=r(gini)
		fastgini `depvar' if `touse' [`weight'`exp']
		local res_FGorg=r(gini)
        ...
        if(`bootstrap' & `bootstrap'>0){
			...
			forvalues i=1/`bootstrap'{
    			...
    			qui: keep if `touse'
    			bsample, `bootopt'
				iopgini `depvar' `indepvars' if `touse' [`weight'`exp']
				return list
				if(`i'==1){
...
if("`shapley'"!=""){
	...
	if((`case'==1 & inlist("`shapley'","ws","pdb")) | (`case'==2 & "`shapley'"=="pdb_max") | (`case'==3 & inlist("`shapley'","fg1a","fg1r","fg2r"))){
		preserve
		iopgini_shapley [`weight'`exp'] if `touse',stat(`shapley') indepvars(`indepvars') depvars(`depvar') gr(`sgroups') nologlin(`nologlin')
		...
		forvalues i=1/`G'{
			...
			di as text "comb: `i'-`j'"
				if(`case'==1){
					...
					fastgini _yhat_gr`i' if `touse' & `groupvar'==`l' [`weight'`exp']
					local value=r(gini)
				}
				di as result "  `value'"
...
capture program drop iopgini_shapley
program define iopgini_shapley , eclass
version 9.2
...
quietly{
	...
	preserve
		...
		local thisvars=""
			...
			//noisily di "iopgini `depvars' `thisvars' [`weight'`exp'], `nologlin'
			iopgini `depvars' `thisvars' [`weight'`exp'], `nologlin'
